...
$n = 51$ Dartmouth undergraduates. Three sign-tests hold in the
...
differ in mean layup ratio, but only marginally ($F = 2.29$,
$p = 0.062$), and the predicted ordering does not hold. Two findings
cut sharply against the model. Pre-medical students show one of the
\emph{lowest} observed layup ratios despite the highest $w$
(suggestive only, $n = 3$), and the empirical population clusters at
the balancer vertex rather than the strategist equilibrium predicted
at $\bar{w} = 3.59$. We discuss explanations and show that eliminating
...
against a survey of $n = 51$ Dartmouth undergraduates. The sign-tests
...
\label{sec:cont-ess}
Because $\pi_i$ is linear in $s_i$, the population mean $\bar s$ is
pulled toward the value at which a layup earns only the baseline payoff,
$w - \gamma \bar s - \varepsilon = 0$, giving
\begin{equation}
  s^\ast = (w - \varepsilon) / \gamma,
  \label{eq:cont_ess}
\end{equation}
valid whenever $0 < w - \varepsilon < \gamma$. When
$s^\ast \in (s_A, s_C)$, the dynamics on $\Delta^2$ settle on an edge
mixture whose mean equals $s^\ast$. When $s^\ast > s_C$, the mean is
clipped at $s_C$ and the population is driven to the strategist
(C) vertex. \ref{app:stability} explains why $s^\ast$ is only
neutrally stable and traces out a one-dimensional rest set rather than
an isolated point.
...
A population sitting at one corner of $\Delta^2$ is stable when no rare
alternative type can invade and grow. Applying this invasion test to
the strategist (C) vertex gives a clean closed-form condition: a
population of strategists cannot be invaded by either purists or
balancers once GPA matters enough,
\begin{equation}
  w > \gamma s_C + \varepsilon = 2.60.
  \label{eq:threshold}
\end{equation}
\ref{app:stability} states the formal ESS criterion, derives
the invasion fitnesses, and tabulates the stability of all three
vertices across $w$.

The three regimes are easiest to read off the phase portraits. At
$w = 1$ (low GPA importance) no corner is stable, and trajectories
settle near the A--B edge with mean close to the continuous rest point
$s^\ast = 0.167$. At $w = 3$ and $w = 5$ the strategist vertex is the
unique stable outcome, with convergence faster at $w = 5$.
...
\begin{figure}[H]
  ...
  \caption{Phase portrait on $\Delta^2$ at $w = 1$ (below threshold).
    ...
    Euler ($\Delta t = 0.04$, $T = 80$) from 16 interior starting
    points (a uniform $n = 6$ simplex grid plus six near-vertex
    seeds). Blue dots mark initial conditions, red stars mark
...
\begin{figure}[H]
  ...
  \caption{Phase portrait on $\Delta^2$ at $w = 3$ (above threshold
    ...
    All 16 interior trajectories converge to it. Markers and
...
We administered a Google Form survey to $n = 51$ Dartmouth
undergraduates during Spring term 2026, with the form distributed
through classmates and peer networks. The
sample skews toward Finance/consulting ($n = 25$, 49\%) and Economics
majors ($n = 14$, 27\%). The mean GPA-importance rating is
$\bar{w} = 3.59$ on a 1--5 scale. The distribution of $s$ is
right-skewed, with most respondents between 0--40\% layups and a
smaller tail above 50\%. Mean layup ratios range from
$\bar{s} = 0.148$ (graduate/PhD) to $\bar{s} = 0.354$
...
\begin{figure}[H]
  ...
  \caption{Survey sample ($n = 51$). Top row: distribution of layup
...
\parencite{long2000using}. The estimate is $\hat\beta_w = +0.0094$
(95\% CI $[-0.059, +0.078]$, $p = 0.79$). The sign matches the
prediction, but the effect is statistically indistinguishable from
zero, and with 17 fixed-effect indicators on $n = 51$ the
specification is over-parameterized (adjusted $R^2 = -0.026$), so the
slope is attenuated and imprecisely estimated. Treating the
...
$\hat\gamma = 1/\hat\beta_w \approx 106$. Because the 95\% CI for
...
competition. Dropping the seven double-major
respondents leaves the sign unchanged
($\hat\beta_w = +0.030$, $p = 0.45$).

\subsection{P2: Cross-Track Distribution of Layup Ratios}

We regress $s_i$ on career-track indicators with Undecided/other as
the baseline (HC1 errors). The overall $F$-test is only marginally
significant ($F = 2.29$, $p = 0.062$), and Table~\ref{tab:track} shows the
ordering is not what the model predicts. Finance/consulting ranks
highest ($\bar s = 0.354$), followed by Tech/engineering (0.268),
Government/nonprofit (0.258), Undecided/other (0.247), Pre-medical
(0.167), and Graduate/PhD (0.148). Pre-medical students, whom the
model identifies as the most strategic (highest mean $w = 4.33$,
$n = 3$), exhibit one of the \emph{lowest} observed layup ratios. No
track coefficient is significant against the baseline.
Finance/consulting comes closest
($\hat\beta = +0.107$, $p = 0.25$, Figure~\ref{fig:p2}).

\begin{table}[H]
\centering
\caption{Track-level statistics, sorted by mean layup ratio.
$\hat\beta$ and $p$-value are OLS coefficients relative to
Undecided/other, HC1 standard errors. The small-$n$ tracks
(Pre-medical and Graduate/PhD, $n = 3$ each) carry little
inferential content.}
\label{tab:track}
\small
...
\midrule
Finance/consulting   & 25 & 0.354 & 0.165 & $+0.107$ & 0.253 \\
Tech/engineering     &  9 & 0.268 & 0.184 & $+0.021$ & 0.846 \\
Government/nonprofit &  4 & 0.258 & 0.093 & $+0.011$ & 0.913 \\
Undecided/other      &  7 & 0.247 & 0.234 & (baseline) & n/a \\
Pre-medical          &  3 & 0.167 & 0.153 & $-0.081$ & 0.487 \\
Graduate/PhD program &  3 & 0.148 & 0.128 & $-0.099$ & 0.360 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Distribution of layup ratio $s$ by career track (descending
    ...
    is only marginally significant ($F = 2.29$, $p = 0.062$). Contrary
    to the model, the highest-$w$ tracks (Pre-medical, Graduate/PhD)
    show among the lowest means.}
  \label{fig:p2}
\end{figure}
...
(Q7 $\geq 1$, $n = 25$) show higher $s$ and $w$ than those
who never avoided ($n = 26$). The differences are
$E[s\mid\text{avoided}] - E[s\mid\text{never}] = +0.070$ and
$E[w\mid\text{avoided}] - E[w\mid\text{never}] = +0.258$, both with
the predicted sign. The $s$ difference is marginally significant
(one-sided Mann--Whitney $p = 0.057$) while the $w$ difference is not
($p = 0.26$).
...
between Q9 and $s$ is $\hat\rho = +0.191$ (one-sided $p = 0.09$,
two-sided $p = 0.18$, $n = 51$), a small positive association in the
...
\begin{figure}[H]
  ...
  \caption{Scatter of $s$ against perceived environmental pressure
    ...
    Spearman $\hat\rho = +0.191$ ($p = 0.18$, $n = 51$).}
  \label{fig:p4}
\end{figure}
...
track and plot the result on $\Delta^2$ (Figure~\ref{fig:simplex_emp}).
No track sits at the C-vertex. Finance/consulting, Tech/engineering,
and Undecided/other are the tracks with a nonzero strategist share
(16\%, 22\%, and 14\% respectively). Finance/consulting also has the
largest balancer mass (76\%). The remaining three tracks have zero
strategists: Government/nonprofit, Graduate/PhD, and Pre-medical
($n = 3$) cluster along the A--B edge with majority-B mixtures. The
model, calibrated at $\gamma = 3.0$ and evaluated at $\bar w = 3.59$,
predicts convergence to the C-vertex. The data show the opposite. This
discrepancy is our most substantive empirical finding.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{empirical_simplex.png}
  \caption{Empirical population state by career track on $\Delta^2$
    (type defined by layup ratio). No track reaches the C-vertex. The
    tracks lie in or near the B-region with at most a 22\% strategist
    share (Tech/engineering), contrary to the model's prediction of
    C-vertex dominance at $\bar w = 3.59$.}
...
final sample is $n = 51$, with all respondents reporting complete
...
lies strictly between the purist and strategist ratios,
$s_A < s^\ast < s_C$, over $w \in (0.8, 2.6)$, keeping the replicator
dynamics non-trivially interior over roughly the lower two-fifths of
the survey's $w \in [1, 5]$ range, including the
C-vertex ESS threshold $w^\ast = \gamma s_C + \varepsilon = 2.60$.
...
re-normalized. Vertex stability was computed analytically
(\ref{app:stability}). P1 and P2 use OLS with HC1
standard errors \parencite{long2000using}.\footnote{HC1 is the
heteroscedasticity-consistent (Eicker--Huber--White) ``robust''
covariance estimator with the finite-sample scaling $n/(n-k)$. It is
appropriate here because the outcome $s_i$ is a bounded ratio in
$[0,1]$, whose error variance cannot be constant---it is compressed
near the $0$ and $1$ boundaries and widest in the interior---so the
homoskedasticity assumption behind classical OLS standard errors
fails. HC1 leaves the coefficient point estimates unchanged and
rescales only the standard errors, keeping the reported $p$-values
valid under this non-constant variance. The $n/(n-k)$ factor is a
conservative small-sample adjustment. This is standard practice for
inference with fractional outcomes.} Although $s_i$ is bounded
...
Against 51 survey responses, three of the four directional predictions
(P1, P3, P4) hold in sign but fall short of conventional significance.
P2's cross-track $F$-test is only marginally significant ($p = 0.062$),
and the predicted ordering does not hold. Two substantive anomalies
stand out, the pre-medical reversal and the population's concentration
at the B-vertex.
...
($\bar w = 4.33$) but contradict the second ($\bar s = 0.167$, among
the lowest of any track). With $n = 3$, no inference about the group
mean is warranted. The more substantive possibility is that pre-meds use a
...
With mean $\bar w = 3.59$ above the threshold $w = 2.60$, the
...
$1/\hat\beta_w \approx 106$ is formally unidentified given the
...
\parencite{weitz2016oscillating, tilman2020evolutionary}, in which the
replicator equation is coupled to an environmental variable for grading
stringency. Such systems can produce persistent oscillations between
``conservation'' and ``exploitation'' regimes that no decoupled
replicator model can capture, and we sketch the coupled system in
\ref{app:stability}. The multi-decade decline in study time
documented by \textcite{babcock2011falling} is qualitatively consistent
with the descending arc of such a cycle. Institutional reforms act
primarily on the environmental dynamics, not on $\gamma$ directly, which
makes the decoupled counterfactual a conservative guide.

\paragraph{Limitations and future directions.}
The sample ($n = 51$) is a single-institution convenience sample
...
\section{Stability Analysis and Invasion Fitness}
\label{app:stability}
% ============================================================

This appendix collects the formal stability machinery summarized in
Section~\ref{sec:cont-ess} and the Results.

\paragraph{ESS criterion.}
...
\paragraph{Invasion fitnesses and the C-vertex threshold.}
Linearizing the replicator system (Eq.~\ref{eq:replicator}) at vertex
$i$ on the simplex tangent yields two non-trivial eigenvalues,
\begin{equation}
...
$w > \gamma s_C + \varepsilon = 2.60$ (Eq.~\ref{eq:threshold}).
Computed this way, Eq.~\ref{eq:invasion} gives the two tangent-space
eigenvalues directly and avoids the spurious off-simplex eigenvalue of
a naive 3D finite-difference Jacobian.

\paragraph{Why $s^\ast$ is only neutrally stable.}
At $\bar s = s^\ast$ every strategy earns the baseline payoff
$\varepsilon$, so $s^\ast$ is a Nash equilibrium of the auxiliary
continuous-strategy game (where $s \in [0,1]$ is unconstrained) but
only neutrally stable along the strategy axis. It is not a strict ESS
in the Maynard-Smith sense, since linearity of $\pi_i$ in $s_i$ yields
indifference rather than a strict invasion inequality. The three-type
discrete game admits no analogous \emph{isolated} interior rest point:
since $\pi_i - \pi_j = (s_i - s_j)(w - \gamma \bar s - \varepsilon)$,
all three payoffs coincide exactly on the line $\bar s = s^\ast$, which
yields a one-dimensional rest set rather than a single point.

\paragraph{Vertex classification across $w$.}
Table~\ref{tab:stability} reports the resulting classification at
$w \in \{1, 3, 5\}$. At $w = 1$ no corner is stable, with A and C
unstable sources and B a saddle. At $w = 3$ and $w = 5$ the C-vertex is
the unique stable ESS, with convergence faster at $w = 5$ (more
...
\paragraph{Outlook: coupled environment--strategy dynamics.}
A complete model would couple the replicator equation to an
environmental variable $n$ representing grading stringency,
\begin{equation}
...
payoff differential as a function of stringency, and $g, h$ capture how
collective layup-taking erodes grading discipline.
