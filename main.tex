...
importance is associated with higher layup ratios in sign (though not
at conventional significance), career tracks differ
...
highest $w$ (suggestive only; $n = 2$), and the empirical population
clusters at the balancer vertex rather than the strategist equilibrium
predicted at $\bar{w} = 3.57$. We discuss explanations and show that
...
approximately equal to the continuous-game ESS $s^\ast = 0.167$,
...
zero given $n = 42$ and 16 fixed-effect indicators. Treating the
...
between Q9 and $s$ is $\hat\rho = +0.231$ (two-sided $p = 0.14$,
$n = 42$), a small positive association in the predicted direction. Figure~\ref{fig:p4} shows the scatter with horizontal
...
The implied $\hat\gamma \approx 97$, though formally unidentified
given the insignificant slope, would push the interior fixed point to
$w \approx 49$, generating interior
